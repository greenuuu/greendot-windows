...
namespace ShadowSocks.Controller
{
    static class AutoStartup
    {
        ...
        private static string Key = "greendot_" + Application.StartupPath.GetHashCode();
            ...
            try
            {
                ...
                foreach (string item in runList)
                {
                    if (item.Equals(Key, StringComparison.OrdinalIgnoreCase))
                        return true;
                    else if (item.Equals("GreenDot", StringComparison.OrdinalIgnoreCase)) // Compatibility with older versions
                    {
